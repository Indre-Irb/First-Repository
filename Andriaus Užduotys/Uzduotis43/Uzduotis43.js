...
//     box.innerHTML += `<h1>${jokes[i].category}</h1>
// <div>${jokes[i].joke}</div>
...
const about = document.querySelector(".movie")
...
for (let i = 0; i < movies.length; i++) {
    about.innerHTML += `
    <div class="display">
        <img class="imgStyle" src="${movies[i].image}" alt="">
            <div>
                <div>${movies[i].titleLtu}</div>
                <div><i class="fab fa-imdb"></i> ${movies[i].imdbRating}</div>
                <div>${movies[i].year}</div>
                <div>${movies[i].genre}</div>
...
select.onchange = () => {
    about.innerHTML = "";
    for (let i = 0; i < movies.length; i++) {
        if (movies[i].genre.includes(select.value)) {
            about.innerHTML += `<div class="display">
        <img class="imgStyle" src="${movies[i].image}" alt="">
            <div>
                <div>${movies[i].titleLtu}</div>
                <div><i class="fab fa-imdb"></i> ${movies[i].imdbRating}</div>
...
const btn = document.querySelector("button")
    const showValue = document.getElementById("slider_value")

function show_value(x) {
    showValue.innerHTML = `${Number(slider.value).toFixed(1)}`;

    btn.onclick= () => {
        about.innerHTML = "";
        for (let i = 0; i < movies.length; i++) {
        if(slider.value <= movies[i].imdbRating){
            about.innerHTML += `<div class="display">
        <img class="imgStyle" src="${movies[i].image}" alt="">
            <div>
                <div>${movies[i].titleLtu}</div>
...
function fl1 (year){
    const flt = movies.filter(x => x.year === year)
    console.log(flt)
}
...
function fl2 (rating){
    const flt = movies.filter(x => x.imdbRating === rating)
    console.log(flt)
}
...
function fl3 (dsp1, dsp2) {
    const flt = movies.filter(x => x.description.length >= dsp1 && x.description.length <= dsp2)
    console.log(flt)
}
fl3(100, 200)
...
function fl4 (gnr) {
    const flt = movies.filter(x => x.genre.includes(gnr))
    console.log(flt)
}
